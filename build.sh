...
# script url
if [ "$isCN" = "CN" ]; then
    export mirror=raw.githubusercontent.com/kousyougi/r4s_build_script/master
else
    export mirror=raw.githubusercontent.com/kousyougi/r4s_build_script/master
fi
...
if [ "$1" = "rc2" ]; then
    ...
    #2024.10.24# git clone https://$github/openwrt/routing master/routing --depth=1
fi
...
if [ "$1" = "rc2" ]; then
    ...
    #2024.10.24#routing="^$(grep routing feeds.conf.default | awk -F^ '{print $2}')"
    #2024.10.24#telephony="^$(grep telephony feeds.conf.default | awk -F^ '{print $2}')"
else
    packages=";$branch"
    #2024.10.24# luci=";$branch"
    #2024.10.24#routing=";$branch"
    #2024.10.24#telephony=";$branch"
fi
cat > feeds.conf <<EOF
src-git packages https://$github/openwrt/packages.git$packages
#2024.10.24# src-git luci https://$github/openwrt/luci.git$luci
#2024.10.24# src-git routing https://$github/openwrt/routing.git$routing
#2024.10.24# src-git telephony https://$github/openwrt/telephony.git$telephony
EOF
